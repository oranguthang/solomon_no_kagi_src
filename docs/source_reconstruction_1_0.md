...
- Every stream-classified PRG byte has exactly one decoder/encoder owner. Room,
  ...
  image.
- Eight deterministic FCEUX scenarios cover cold boot, normal room entry,
  movement, both casting paths, room completion, scheduler/timer behavior, and
  audio channel priority. Controlled writes are declared in the scenario
  manifest and checked by the validator.
...
eight runtime scenarios. A previously generated trace cannot satisfy this gate
