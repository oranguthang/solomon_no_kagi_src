...
`src/graphics/hud/gameplay.asm` owns CPU `$C3D4-$C3FD`, the five-byte fairy
template owns `$C3FE-$C402`, and `src/graphics/hud/score.asm` owns
...
the return, header data, and entry distinct in `src/game/items/collision.asm`.
