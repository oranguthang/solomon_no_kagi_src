...
def validate_tool_layout(root: Path) -> None:
    ...
    if flat_tools:
        raise ProjectError("uncategorized scripts: " + ", ".join(flat_tools))
    for category in sorted(categories):
        if not (script_root / category / "__init__.py").is_file():
            raise ProjectError(f"missing script package: scripts/{category}")
...
def command_lint(_args: argparse.Namespace) -> None:
    required = (
        ...
        "scripts/validation/prg_layout.py",
        "scripts/runtime/runtime_scenarios.py",
        "scripts/runtime/capture_runtime_scenario.lua",
        "scripts/validation/reconstruction_status.py",
        "scripts/validation/enemy_ai_data.py",
        "scripts/validation/item_handler_data.py",
        "scripts/validation/enemy_pointer_data.py",
        "scripts/validation/scheduler_data.py",
        "scripts/authoring/title_data.py",
        "scripts/build/verify_rom.py",
        "src/main.asm",
