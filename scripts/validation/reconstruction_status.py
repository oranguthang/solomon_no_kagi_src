...
HISTORICAL_TOOL_PATHS = {
    ...
    "scripts/revision_profiles.py": "scripts/build/revision_profiles.py",
    "scripts/level_editor.py": "scripts/authoring/level_editor.py",
    "scripts/level_preview.py": "scripts/authoring/level_preview.py",
    "scripts/level_studio.py": "scripts/authoring/level_studio.py",
    "scripts/room_data.py": "scripts/authoring/room_data.py",
    "scripts/audio_data.py": "scripts/authoring/audio_data.py",
    "scripts/audio_editor.py": "scripts/authoring/audio_editor.py",
    "scripts/audio_preview.py": "scripts/authoring/audio_preview.py",
    "scripts/sound_studio.py": "scripts/authoring/sound_studio.py",
    "scripts/graphics_editor.py": "scripts/authoring/graphics_editor.py",
    "scripts/graphics_studio.py": "scripts/authoring/graphics_studio.py",
    "scripts/presentation_editor.py": "scripts/authoring/presentation_editor.py",
    "scripts/presentation_studio.py": "scripts/authoring/presentation_studio.py",
}
...
def release_path(project_root: Path, relative: str) -> Path:
    """Resolve immutable historical manifest paths after an audited tool move."""
    path = project_root / relative
    if path.exists():
        return path
    return project_root / HISTORICAL_TOOL_PATHS.get(relative, relative)
...
def validate_release_evidence(
    ...
    for identifier, requirement in requirements.items():
        ...
        for relative in evidence.get("files", []):
            evidence_count += 1
            if not release_path(project_root, str(relative)).exists():
...
def validate_release_v3_paths(project_root: Path, release: dict[str, Any]) -> list[str]:
    ...
    for field in ("required_documents", "required_files"):
        ...
        for relative in paths:
            if not release_path(project_root, relative).exists():
    ...
    if not isinstance(excluded_scope, list) or not excluded_scope:
        ...
        for entry in excluded_scope:
            ...
            for relative in entry.get("evidence", []):
                if not release_path(project_root, str(relative)).is_file():
    ...
    if not isinstance(delta, list) or not delta:
        ...
        for entry in delta:
            ...
            for value in evidence:
                if value not in known_evidence and not release_path(
